...
# Periodic commits during large exports (same connection; idempotency prevents dup rows on resume).
_APPLE_IMPORT_COMMIT_EVERY = 1000
...
def _ingest_apple_workout(
    ...
    if idempotency_seen(cur, CONNECTOR_APPLE, dk):
...
def ingest_apple_health_export_from_zip(database: Database, zip_path: Path) -> dict[str, Any]:
    ...
    def bump_commit() -> None:
        ...
        if writes_since_commit >= _APPLE_IMPORT_COMMIT_EVERY:
            cx.commit()
            writes_since_commit = 0
        ...
        with zipfile.ZipFile(zip_path) as zp:
            ...
            with zp.open(member, "r") as raw_xml:
                for _event, elem in ET.iterparse(raw_xml, events=("end",)):
                    ...
                    if tag == "Workout":
                        _ingest_apple_workout(cur, elem, stats, bump_commit)
                        continue
                    if tag != "Record":
                        continue
