...
class Settings(BaseSettings):
    ...
    # Optional override for OpenClaw health-coach SQLite (skills/health-coach/data/health.db).
    ...
    def resolved_health_db(self) -> Path:
        if self.health_db_path is not None:
            return Path(self.health_db_path).expanduser().resolve()
        repo_root = _SETTINGS_DIR.parent.parent
        return (repo_root / "vendor/openclaw-health/skills/health-coach/data/health.db").resolve()
